...
This post documents the configuration end to end. It covers the installation of `ttyd`, Tailscale, and tmux on an Ubuntu 24.04 laptop; the binding of the `ttyd` listener to the Tailscale network interface; the wrapping of the service in a per-user systemd unit that survives logout and reboot; the client-side configuration on iOS; and optional hardening with a TLS certificate issued by `tailscale cert`. Appendices cover a sample work session and the full teardown procedure. Corrections and alternative approaches are welcome.

More formally, this post documents an entry point to the Remote Access concern of the Workflow Construct described in [post 52](/posts/52-workflow-construct/). Tailscale plus `ttyd` is one specific solution to the cross-machine, cross-network reachability problem; the broader Remote Access family also includes `mosh` for high-latency-link resilience and standard SSH for the unauthenticated-network case. This post is the browser-shell-on-mobile leaf of that family.
...
::: {.callout-note}
If your distribution ships a `ttyd` older than 1.7, install a prebuilt binary from the [ttyd releases page](https://github.com/tsl0922/ttyd/releases) instead. Earlier versions lack the `-i` interface-binding flag used below.
:::
...
- The single most consequential line in the configuration is `-i tailscale0` on the `ttyd` invocation. This binding decision is what removes the service from every non-tailnet interface and makes the rest of the security model meaningful.
- A per-user systemd unit combined with `loginctl enable-linger` provides a logout-safe and reboot-safe service without any system-level privileges beyond the initial `linger` enablement.
- The `tmux new -A -s main` argument inside the `ExecStart` directive is the mechanism that transforms an ephemeral browser tab into a durable working session.
- The Safari **Add to Home Screen** action yields a standalone web-application presentation that recovers enough screen area to make the terminal practically usable on phone-scale displays.
