...
- **Interface binding (`ttyd -i tailscale0`).** The listener is bound to the Tailscale virtual interface rather than to `0.0.0.0`. If the tailnet were somehow compromised at the coordination-server level, or if Tailscale itself were misconfigured to route traffic from unexpected sources into `tailscale0`, the service would still be unreachable from the physical interfaces. This is a defense-in-depth control.
